...
export default function CompleteProfile({ user: userData = {} }) {
   ...
   const activeUser =
      Object.keys(userData).length > 0
         ? userData
         : Array.isArray(contextUser)
         ? contextUser[0]
         : contextUser;

   const { fields, completionPercentage, isLoading } =
      useCompleteProfileLogic(activeUser);
   ...
   return (
      <section
         className="flex flex-col gap-6 p-6 bg-main dark:bg-zinc-900 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-800"
